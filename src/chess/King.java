...
public class King {
	...
	public static Message IsMoveValidCheck(PieceType piece_type, PieceFile cur_file, int cur_rank, PieceFile mov_file,
		...
		if (piece_type == PieceType.WK) {
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, left_file_bound, top_rank_bound)
					|| IsValidPosition(piece_type, mov_file, mov_rank, left_file_bound, top_rank_bound)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, left_file_bound, cur_rank)
					|| IsValidPosition(piece_type, mov_file, mov_rank, left_file_bound, cur_rank)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, left_file_bound, bot_rank_bound)
					|| IsValidPosition(piece_type, mov_file, mov_rank, left_file_bound, bot_rank_bound)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, cur_file, top_rank_bound)
					|| IsValidPosition(piece_type, mov_file, mov_rank, cur_file, top_rank_bound)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, cur_file, bot_rank_bound)
					|| IsValidPosition(piece_type, mov_file, mov_rank, cur_file, bot_rank_bound)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, right_file_bound, top_rank_bound)
					|| IsValidPosition(piece_type, mov_file, mov_rank, right_file_bound, top_rank_bound)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, right_file_bound, cur_rank)
					|| IsValidPosition(piece_type, mov_file, mov_rank, right_file_bound, cur_rank)) {
				return null;
			}
			if (IsAppropriatePieceCheck(piece_type, mov_file, mov_rank, right_file_bound, bot_rank_bound)
					|| IsValidPosition(piece_type, mov_file, mov_rank, right_file_bound, bot_rank_bound)) {
				return null;
			}
		} else if (piece_type == PieceType.BK) {
