...
public class Pawn {
	public static Message IsMoveValid(PieceType piece_type, PieceFile cur_file, int cur_rank, PieceFile mov_file,
			int mov_rank, boolean enPassant) {
		...
		if (piece_type == PieceType.WP) {
			for (ReturnPiece rp : Chess.return_play.piecesOnBoard) {
				...
				if (rp.pieceFile == right_file_bound && rp.pieceRank == top_rank_bound) {
					if (mov_file == right_file_bound && mov_rank == top_rank_bound
				...
				if (rp.pieceFile == left_file_bound && rp.pieceRank == top_rank_bound) {
					if (mov_file == left_file_bound && mov_rank == top_rank_bound
							&& rp.pieceType.toString().startsWith("B")) {
						Chess.return_play.piecesOnBoard.remove(rp);
						return null;
					}
			...
			for (ReturnPiece rp : Chess.return_play.piecesOnBoard) {
				...
				if (rp.pieceFile == right_file_bound && rp.pieceRank == bottom_rank_bound) {
					if (mov_file == right_file_bound && mov_rank == bottom_rank_bound
				...
				if (rp.pieceFile == left_file_bound && rp.pieceRank == bottom_rank_bound) {
					if (mov_file == left_file_bound && mov_rank == bottom_rank_bound
							&& rp.pieceType.toString().startsWith("W")) {
						Chess.return_play.piecesOnBoard.remove(rp);
						return null;
					}
	...
	public static boolean CheckForPromotion(PieceType piece_type, int mov_rank) {
		if (piece_type == PieceType.WP) {
			if (mov_rank == 8) {
				return true;
			}
	...
	private static boolean IsEnPassant(PieceType piece_type, PieceFile cur_file, int cur_rank, PieceFile mov_file,
			int mov_rank) {
		if (cur_rank == 5) {
			if (piece_type == PieceType.WP) {
				for (ReturnPiece rp : Chess.return_play.piecesOnBoard) {
